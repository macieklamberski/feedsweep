...
// `poster` names the image the player shows.
...
const readVideoId = (parsed: URL): string | undefined => {
  ...
  if (isHostOrSubdomainOf(parsed, streamHosts)) {
...
export const cloudflarestreamResolveEmbed: ResolveEmbed = (url) => {
  ...
  const accountCode = parsed.hostname.match(accountCodeRegex)?.[1]
  ...
  return {
    ...
    id: accountCode ? `${accountCode}/${videoId}` : videoId,
