...
const readFileName = (documentUrl: string): string | undefined => {
  ...
  const segment = getPathSegments(parsed).at(-1)
...
export const officeResolveEmbed: ResolveEmbed = (url) => {
  ...
  // No id. The whole payload is the document's own url, and no Microsoft endpoint takes one:
  // the viewer renders a document rather than describing it. An id here would make the
  // placeholder advertise an enrichment key nothing can call, so the field is left unset and
  // `enrichEmbedPlaceholders` passes over it.
  return {
    provider,
    src: parsed.href,
    url: documentUrl,
