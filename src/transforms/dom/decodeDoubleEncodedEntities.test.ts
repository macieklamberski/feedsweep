...
describeForEachParser('decodeDoubleEncodedEntities', (parseHtml) => {
  ...
  describe('leaves single-encoded and unrelated text alone', () => {
    it('should leave a url query beside a doubled entity alone', async () => {
      const value = '<p>see https://x.com/?id=1&amp;param=2&amp;copy=3 and Tom &amp;amp; Jerry</p>'
      const expected = '<p>see https://x.com/?id=1&amp;param=2&amp;copy=3 and Tom &amp; Jerry</p>'

      expect(await transform(value)).toEqualHtml(expected)
    })

    it('should leave a doubled named entity outside the list alone', async () => {
      const value = '<p>Tom &amp;amp; Jerry&amp;nbsp;</p>'
  ...
  describe('respects opaque elements', () => {
    ...
    it('should not decode inside a kbd element', async () => {
      const value = '<p>press <kbd>&amp;amp;</kbd></p>'

      expect(await transform(value)).toEqualHtml(value)
    })

    it('should not decode inside a samp element', async () => {
      const value = '<p>prints <samp>&amp;amp;</samp></p>'
